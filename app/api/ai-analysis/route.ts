...
// Only initialize the OpenAI client when an API key is available.
...
async function generateAIInsights(url: string, htmlContent: string, currentChecks: any[]) {
  // Try Groq first since OpenAI may have quota/cost concerns.
  const groqResult = await generateGroqInsights(url, htmlContent, currentChecks);

  // If Groq returns meaningful insights, use them. Otherwise, fallback to OpenAI if configured.
  if (groqResult && groqResult.insights && groqResult.insights.length > 0) {
    return groqResult;
  }

  if (openai) {
    try {
      const openaiResult = await generateOpenAIInsights(url, htmlContent, currentChecks);
      if (openaiResult && openaiResult.insights && openaiResult.insights.length > 0) {
        return openaiResult;
      }
    } catch (e) {
...
async function generateOpenAIInsights(url: string, htmlContent: string, currentChecks: any[]) {
  ...
  // SDK response shape: resp.choices[0].message.content
  const content = resp?.choices?.[0]?.message?.content;
  if (!content) throw new Error('Empty OpenAI response');

  let trimmed = String(content).replace(/```json\n?/g, '').replace(/```\n?/g, '').trim();
  try {
    return JSON.parse(trimmed);
  } catch (err) {
    console.error('Failed to parse OpenAI JSON:', err);
    console.log('OpenAI raw content:', trimmed.substring(0, 200));
    throw err;
  }
}
